...
class SignUpControllerFactoryTests: XCTestCase {
    ...
    func test_signUp_compose_with_correct_validations() {
        ...
        XCTAssertEqual(validations[3] as! RequiredFieldValidation, RequiredFieldValidation(fieldName: "passwordConfirmation", fieldLabel: "Confirmar Senha"))
        XCTAssertEqual(validations[4] as! EmailValidation, EmailValidation(fieldName: "email", fieldLabel: "Email", emailValidator: EmailValidatorSpy()))
...
extension SignUpControllerFactoryTests {
    ...
    func makeSut(file: StaticString = #filePath, line: UInt = #line) -> (SignUpViewController, AddAccountSpy) {
        ...
        let sut = makeSignUpControllerWith(addAccount: MainQueueDispatchDecortator(addAccountSpy))
